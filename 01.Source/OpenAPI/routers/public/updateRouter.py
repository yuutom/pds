...
# 処理名
EXEC_NAME: str = "update"
EXEC_NAME_JP: str = "個人情報更新"
...
def advance_exec(
    ...
    Args:
        ...
        info_item_flg_dict (dict): 情報項目有無フラグ情報
    """
    try:
        # 01.PDSユーザドメイン検証処理実行
        ...
        input_check_result = input_check(trace_logger, pds_user_domain_name, request_body, pds_user_id, access_token, time_stamp, pds_user_domain_check_result, info_item_flg_dict)
...
def input_check(
    ...
    try:
        ...
        check_image_result = checkImage.CheckImage(trace_logger, request_body.info.image, 2).get_result()
        ...
        # 01-03-06.保存されたバイナリデータハッシュ値検証処理
        # 01-03-06-01.以下の引数で保存されたバイナリデータハッシュ値検証処理を実行する
        check_image_hash_result = checkImageHashTo.CheckImageHashTo(trace_logger, request_body.info.imageHash, 2).get_result()
        ...
        if request_body.info.imageHash is None:
            ...
            image_hash = request_body.info.imageHash
        if len(image) != len(image_hash) or info_item_flg_dict["image"] is not info_item_flg_dict["imageHash"]:
...
# 個人情報更新API
@router.put("/api/2.0/{pdsUserDomainName}/transaction")
async def update(
    request: Request,
    ...
    try:
        ...
        # リクエスト要素有無判定
        info = request._json["info"]
        info_item_flg_dict = {
            "userId": "userId" in info,
            "saveDate": "saveDate" in info,
            "data": "data" in info,
            "image": "image" in info,
            "imageHash": "imageHash" in info,
            "secureLevel": "secureLevel" in info
        }
        # 01 事前処理
        # 01-01 以下の引数で事前処理を実行
        # 01-02 事前処理からのレスポンスを、「変数．事前処理実行結果」に格納する
        advance_exec_result = advance_exec(trace_logger, guid, pdsUserDomainName, request_body, pdsUserId, accessToken, timeStamp, jwt, request, info_item_flg_dict)
        trace_logger = advance_exec_result["traceLogger"]
        out_logger = advance_exec_result["outLogger"]

        # 個人情報更新処理
        update_model = updateModel(advance_exec_result["traceLogger"], request, pdsUserId, advance_exec_result["pdsUserInfo"], pdsUserDomainName, info_item_flg_dict)
        await update_model.main(request_body)

        # 08.アクセストークン発行処理
        # 08-01.以下の引数でアクセストークン発行処理を実行する
        # 08-02.アクセストークン発行処理のレスポンスを「変数．アクセストークン発行処理結果」に格納する
        token_util = TokenUtilClass(trace_logger)
        ...
        try:
            ...
            # 10-02.処理に失敗した場合、以下の引数でAPI実行履歴登録処理を実行する
            common_util.insert_api_history(
                commonUtil.get_datetime_str_no_symbol() + commonUtil.get_random_ascii(13),
                advance_exec_result["pdsUserInfo"]["pdsUserId"],
                apitypeConstClass.API_TYPE["UPDATE"],
