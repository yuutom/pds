...
# 自作クラス
## 業務ロジッククラス
from models.closed.tfOperatorSearchModel import tfOperatorSearchClass
...
EXEC_NAME_JP: str = "TFオペレータ検索・参照"

# ルータ作成
router = APIRouter()


def input_check(trace_logger: Logger, access_token, time_stamp):
    """
    パラメータ検証処理
...
# TFオペレータ検索・参照API
@router.post("/api/2.0/tfoperator/search")
async def tf_operator_search(
    request: Request,
    ...
    try:
        ...
        input_check_result = input_check(trace_logger, accessToken, timeStamp)
        # 04.共通エラーチェック処理
        ...
        tf_operator_search_model = tfOperatorSearchClass(trace_logger)
        main_result = tf_operator_search_model.main()

        # 09.アクセストークン発行処理
        # 09-01 以下の引数でアクセストークン発行処理を実行する
        ...
        response_content = {
            ...
            "tfOperatorInfo": main_result["tfOperatorInfo"]
        }
