...
# 自作クラス
## 業務ロジッククラス
from models.closed.pdsUserGetUseCountModel import pdsUserGetUseCountModelClass
## utilクラス
...
# 処理名
EXEC_NAME: str = "pdsUserGetUseCount"
EXEC_NAME_JP: str = "PDSユーザ利用回数確認"
...
# PDSユーザ利用回数確認API
@router.post("/api/2.0/pdsuser/getUseCount")
async def pds_user_get_use_count(
    request: Request,
    ...
    try:
        ...
        # モデルオブジェクト作成
        pds_user_get_use_count_model = pdsUserGetUseCountModelClass(trace_logger)

        # main関数実行
        main_result = pds_user_get_use_count_model.main(request_body)

        # 15.アクセストークン発行処理
        # 15-01.アクセストークン発行処理を実行する
        ...
        response_content = {
            ...
            "charge": main_result["billing_amount"],
            "pdsUseData": main_result["use_count_data"]
        }
