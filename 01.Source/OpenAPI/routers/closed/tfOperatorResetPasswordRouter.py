...
# 自作クラス
## 業務ロジッククラス
from models.closed.tfOperatorResetPasswordModel import tfOperatorResetPasswordClass
...
EXEC_NAME_JP: str = "TFオペレータパスワードリセット"
...
# TFオペレータパスワードリセットAPI
@router.post("/api/2.0/tfoperator/resetpassword")
async def tf_operator_reset_password(
    request: Request,
    ...
    try:
        ...
        tf_operator_reset_password_model = tfOperatorResetPasswordClass(trace_logger)
        response_content = tf_operator_reset_password_model.main(request_body)

        # 19.アクセストークン発行処理
        # 19-01 以下の引数でアクセストークン発行処理を実行する
